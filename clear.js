 ...
 * version : 1.1    
 * Date : 2012年9月9日   
 * Email : [email]hanan321@qq.com[/email]
 * Site : [url]http://www.cssha.com/[/url] 
 */
...
function getMineSweeper(){
    var oCount = parseInt($('oCount').innerHTML);//获取栏目等级
    ...
    max = y*x;//格子总数
    $('oMain').style.width = 24*x+2+'px';//确定宽度
    for(var i=0; i<y; i++){    //生成格子
        boxslist[i] = [];
        for(var j=0; j<x; j++){
            boxslist[i].push(new CreateBox(i,j,0));//构造雷格对象
        }
    }
    for(var k=0; k<oCount; k++){   //生成随机雷格数组序号
        var new_num = Math.floor(Math.random()*max+1);
        getDifferentNumber(new_num,has,max);//has=[],max = y*x，这里讲随机数进行处理，生成不重复的随机数
        new_num = _differendNumber;
        has.push(new_num);  //放入has数组的是不重复的随机数
    }
    function setMine(arr){
        for(var i=0; i<has.length; i++){
            var mineX = arr[i] % x == 0 ? x : arr[i] % x;
            var mineY = Math.ceil(arr[i] / x);
            boxslist[mineY-1][mineX-1].mine = 1;//将数组里面有雷的定义属性mine为1
        }
    }
    mineArray = has; //不重复的随机数放入地雷组
    overcomeMineLenth.minelength = mineArray.length;//overcomeMineLenth = {overcome:0, minelength:10};赋值
    setMine(has);//给格子里面有雷的元素设置mine值为1
    function getDifferentNumber(n,has,max){    //生成不重复的随机数
        var _has = has;
        var _max = max;
        var _n = n;
        var isin = 0;
        for(var i=0; i<_has.length; i++){//_has第一次等于0，第二次等于1，然后增加
            if(_n==has[i]){//判断生成的随机数是否和之前生成的相同
               isin = 1; 
            }
        }
        if(isin==1){   //如果两次生成的随机数相同，递归执行
    ...
    minesweeper.len = max;//minesweeper雷区对象
    minesweeper.x = x;
    minesweeper.y = y;
...
function mineSweeperToHTML(obj){    //布雷到页面HTML
     var x_len = obj.x;//minesweeper.x
     var y_len = obj.y;//minesweeper.y
     var arr = obj.boxslist;//minesweeper.boxslist
     var str = '';
...
function mineBoxClick(ele){     //格子点击
    ...
    var y = parseInt(ele_id_arr[1]);//获取所在行，列数
    var x = parseInt(ele_id_arr[2]);
    var box = minesweeper.boxslist[y][x];//获取在数组中的属性
    var eleclass = ele.getAttribute('class');//获取当前属性，是点击状态还是插旗还是没有点击
    if(eleclass!='hidden'){
        return false; //如果已经点击，返回false
    }else{
        box.on = 1;//如果没有点击，则给on属性赋值为1 //0未扫，1已扫，2已标记
    }
    // var ele = $('box-'+y+'-'+x);
    if(box.mine==0){    //不是雷 //1有雷，0无
        overcomeMineLenth.overcome += 1; 
        var tl,tc,tr,bl,bc,br,l,r;  //获取周围的格子对象
        var out = {position:0, mine:0}; //position：1在雷区里面，0不在
        var maxY = minesweeper.y - 1;
        var maxX = minesweeper.x - 1;
        if(box.t_l.y < 0 || box.t_l.y > maxY || box.t_l.x < 0 || box.t_l.x > maxX){//this.t_l = {y:y-1, x:x-1};左上格子坐标
            tl = out ;//超出格子的话，表示position:0 不在雷区
        }else{//否则在雷区
        ...
        var round = tl.mine + tc.mine + tr.mine + bl.mine + bc.mine + br.mine + l.mine + r.mine; //获取这些值得mine属性，并且计算总和
        switch(round){  //根据周围格子元素样式设置
            ...
            break; 
            default:
            ele.setAttribute('class','on');     //如果round=0有空格，继续递归搜索
        ...
        $('start').setAttribute('src','http://www.cssha.com/wp-content/uploads/2012/09/sad.gif');
        //移除添加事件
...
function addFlag(ele,event){    //右键标记红旗
    ...
    switch(bumpBOx.mousekey){
        ...
        case 3://双击效果
        switch (_c){
            case 'hidden' ://如果是处于隐藏，插旗，和空白状态下操作无效
            case 'flag' :
            case 'on' :
            case '' :
            return false;
...
function leftRightMouseShow(ele){   //左右键双击效果
    ...
    var l = $('box-'+y+'-'+(x-1));
    var r = $('box-'+y+'-'+(x+1));
    var _flag = getFlag(tl) + getFlag(tc) + getFlag(tr) + getFlag(bl) + getFlag(bc) + getFlag(br) + getFlag(l) + getFlag(r) ;//获取周围旗子的个数
    if(ele.innerHTML == _flag){ //如果按钮数等于插旗数，周围的格子都点开
        isClick(tl);
        isClick(tc);
        isClick(tr);
        isClick(bl);
...
function showDown(arr){     
    ...
    for(var i=0; i<len; i++){
        if(arr[i]){
            ...
            if(_c=='hidden'){
                arr[i].setAttribute('class','onbump');//如果格子是隐藏状态，则显示凹陷效果
            }
        }
    }
    setTimeout("backClass(bumpBOx.bump)",100); //延迟执行，返回凹陷前效果
}
function backClass(arr){    
    ...
    for(var i=0; i<len; i++){
        if(arr[i]){
            var _c = arr[i].getAttribute('class');
            if(_c=='onbump'){
                arr[i].setAttribute('class','hidden');//返回凹陷前效果
            }
        }
    }
}
...
function chooseDifficulty (){   //难度选择
    ...
    for(var i=0; i<len; i++){
        arr[i].onclick = function(){
            ...
            //$('start').setAttribute('src','http://www.cssha.com/wp-content/uploads/2012/09/happy.gif');
            startGame();
        }
    }
}
...
window.onload = function(){//页面加载完毕执行函数
    chooseDifficulty ()
    startGame();
    $('start').onclick = function(){
        this.setAttribute('src','http://www.cssha.com/wp-content/uploads/2012/09/happy.gif');
        getDifficulty();
        startGame();
    }
